...
`kimi-code` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@lcode-cli/lcode`, writes its changelog here:
...
- The released version exists on npm (`npm view @lcode-cli/lcode versions --json`) or has a matching GitHub Release tag.
...
- The upstream H1 `# @lcode-cli/lcode` because the docs page already has `# Changelog`.
...
Drop SDK-only and provider-internal detail. This changelog serves `@lcode-cli/lcode` CLI and web users. Within an entry, keep only what CLI/web users can perceive, and remove sentences that document internals instead of user-visible behavior. Apply this on both the English and Chinese pages:
...
```bash
git log -1 --format=%cs "@lcode-cli/lcode@<version>"
```
