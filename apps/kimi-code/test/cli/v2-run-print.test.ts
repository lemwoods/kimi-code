...
vi.mock('@lcode-cli/agent-core-v2', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@lcode-cli/agent-core-v2')>();
  return {
    ...actual,
...
describe('runV2Print', () => {
  ...
  it('submits a prompt, renders native events, awaits completion, and drains', async () => {
    ...
    expect(stderr.write).toHaveBeenNthCalledWith(1, 'lcode version 1.2.3-test\n');
