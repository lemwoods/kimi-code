...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'lcode');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'lcode')
      : pathWin32.join(home, 'AppData', 'Local', 'lcode', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'lcode');
}
