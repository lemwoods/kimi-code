...
runs the stable TypeScript v1 engine through `@lcode-cli/lcode-sdk` in the
...
flowchart LR
  ...
  SDK["@lcode-cli/lcode-sdk<br/>KimiHarness and Session"]
  Core["v1 agent-core"]
...
- `apps/vscode` depends on `@lcode-cli/lcode-sdk`.
- `apps/vscode` must not depend directly on `@lcode-cli/agent-core`.
...
Migration is opt-in and uses `@lcode-cli/migration-legacy` for detection and
