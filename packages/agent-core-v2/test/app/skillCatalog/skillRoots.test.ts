...
describe('skillRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .lcode/skills directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.lcode/skills/commit'), { recursive: true });

      const roots = await projectRoots(root);

      expect(roots.some((r) => r.path.endsWith('.lcode/skills') && r.source === 'project')).toBe(
        true,
      );
    });
    ...
    it('falls back to the generic .agents/skills directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.lcode/skills'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.lcode/skills'))).toBe(true);
    });

    it('orders the brand directory before the generic directory', async () => {
      await markGitRoot();
      await mkdir(join(root, '.lcode/skills'), { recursive: true });
      await mkdir(join(root, '.agents/skills'), { recursive: true });

      const roots = await projectRoots(root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.lcode/skills'));
