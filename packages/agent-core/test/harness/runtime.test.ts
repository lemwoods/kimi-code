...
`,
  ...
  // does not know the session yet, so reading `.lcode/local.toml` through
...
`,
  ...
  it('adds an additional dir through the session RPC', async () => {
    ...
    const localToml = await readFile(join(workDir, '.lcode', 'local.toml'), 'utf-8');
    const session = core.sessions.get(created.id);
    const mainAgent = session?.getReadyAgent('main');
    ...
    expect(result).toMatchObject({
      ...
      projectRoot: workDir,
      configPath: join(workDir, '.lcode', 'local.toml'),
      persisted: true,
    });
  ...
  it('adds a session-only additional dir without writing local.toml', async () => {
    ...
    await expect(readFile(join(workDir, '.lcode', 'local.toml'), 'utf-8')).rejects.toThrow();
  });
