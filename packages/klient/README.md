...
import { createKlient } from '@lcode-cli/klient/memory';   // or '/ipc'
...
|---|---|---|
| `@lcode-cli/klient/ipc` | `{ socketPath, token? }` | same socket |
| `@lcode-cli/klient/memory` | `{ scope }` (a bootstrapped engine app scope) | direct emitter/bus subscription |
